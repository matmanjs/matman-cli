...
class Builder {
  ...
  static createProdConfig(params = {}, opts = {}) {
    ...
    prodPlugins.push(new CleanWebpackPlugin([params.clientScriptBuildPath], {
      root: projectRoot,
      verbose: true,
      dry: false
    }));
    ...
    // 这里的 entry 需要自动生成
    prodConfig.entry = e2eBuild.getEntry(params.basePath, params.clientScriptBuildPath, params.clientScriptMatch);

    prodConfig.output = {
      filename: '[name].js',
      path: params.clientScriptBuildPath
    };
    prodConfig.module = {
      rules: prodRules
    };
