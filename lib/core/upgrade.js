...
class Upgrade {
  ...
  checkCore() {
    ...
    return pkgJson('matman-cli', 'latest', registry).then(json => {
      ...
      if (!semver.satisfies(version, compatibleVersion)) {
        log.info(`您当前使用的matman版本 ${version}, matman要求使用的版本为 ${compatibleVersion}, 将会使用全量更新策略`);

        const loading = new Loading('正在全量更新cli core，请稍等');
        return self.execNpmCommand('install', 'matman-cli', true).then(function (result) {
          if (!result.code) {
            loading.success();
            log.info(`已经自动升级到matman的最新版本${ltsVersion}`);
          } else {
  ...
  getInstalledPlugins() {
    ...
    return fs.exists(packagePath).then(function (exist) {
      ...
      // Ignore plugins whose name is not started with "matman-plugin-"
      if (!/^matman-plugin-|^@[^/]+\/matman-plugin-|^generator-|^@[^/]+\/generator-/.test(name)) return false;
